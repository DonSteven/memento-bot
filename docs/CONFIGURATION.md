# Memento Bot Configuration and Operations

[Project overview and Quick Start](../README.md#quick-start)

This reference retains the inherited nanobot channel, provider, CLI, SDK, API
and deployment instructions, alongside Memento Bot memory and knowledge settings.
The executable, Python package and configuration directory remain `nanobot`.
Install this repository with the `web_knowledge` extra as shown in Quick Start;
the upstream PyPI package does not install these extensions. Optional channel
and API extras still apply. Third-party service examples are inherited reference
material; live service availability has not been revalidated here.

- [Chat apps](#-chat-apps)
- [Providers](#providers)
- [Memory and knowledge](#semantic-memory-and-external-web-knowledge-models)
...
Connect Memento Bot to your favorite chat platform. Want to build your own? See the [Channel Plugin Guide](CHANNEL_PLUGIN_GUIDE.md).
...
```bash
python -m pip install -e '.[web_knowledge,matrix]'
```
...
```bash
python -m pip install -e '.[web_knowledge,weixin]'
```
...
```bash
python -m pip install -e '.[web_knowledge,wecom]'
```
...
`providers.dashscope.apiKey`. External web knowledge has its own embedding configuration with the same defaults and,
...
Memento Bot uses the inherited **Provider Registry** (`nanobot/providers/registry.py`) as the single source of truth.
...
Memento Bot supports multiple web search providers. Configure in `~/.nanobot/config.json` under `tools.web.search`.
...
Memento Bot supports [MCP](https://modelcontextprotocol.io/) — connect external tool servers and use them as native agent tools.
...
By default, Memento Bot uses `UTC` for runtime time context. If you want the agent to think in your local time, set `agents.defaults.timezone` to a valid [IANA timezone name](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones):
...
Run multiple Memento Bot instances simultaneously with separate configs and runtime data. Use `--config` as the main entrypoint. Optionally pass `--workspace` during `onboard` when you want to initialize or update the saved workspace for a specific instance.
...
Extension evaluation entry points (run `--help` for dataset and output arguments):

| Command | Purpose | Additional install extras |
| --- | --- | --- |
| `nanobot memory-eval --help` | Offline memory replay and extraction checks | None beyond the base setup |
| `nanobot memory-v2-eval --help` | Semantic extraction evaluation | `memory_eval` |
| `nanobot memory-v2-query-eval --help` | Snapshot/replay query evaluation | `memory_eval` |
| `nanobot knowledge-beir-eval --help` | BEIR ranking, evidence and latency reports | `beir_eval` |

Install extras from this checkout, for example `python -m pip install -e '.[web_knowledge,memory_eval,beir_eval]'`.
Live extraction/retrieval evaluations can call paid model services; some evaluation
modes also need dataset or local model downloads. Contract tests use fixed or mocked backends.
...
Use Memento Bot through the `nanobot` library — no CLI, no gateway, just Python:
...
See [docs/PYTHON_SDK.md](PYTHON_SDK.md) for the full SDK reference.
