...
async def test_prompt_below_threshold_does_not_consolidate(tmp_path) -> None:
    ...
    loop.memory_consolidator.consolidate_messages = AsyncMock(return_value=MemoryWriteResult(True, True))  # type: ignore[method-assign]
...
async def test_prompt_above_threshold_archives_until_next_user_boundary(tmp_path, monkeypatch) -> None:
    ...
    await loop.memory_consolidator.maybe_consolidate_by_tokens(session, MemoryContext(), query="")
...
async def test_consolidation_loops_until_target_met(tmp_path, monkeypatch) -> None:
    ...
    call_count = [0]
    def mock_estimate(_session, _memory_context, **_kwargs):
        call_count[0] += 1
        if call_count[0] == 1:
            return (500, "test")
        if call_count[0] == 2:
